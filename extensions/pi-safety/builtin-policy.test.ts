...
import { parseInitialBuiltinAccessPolicy } from "./builtin-policy"
...
test("initial policy protects ambient credentials, project secrets, snapshots, and Pi state", () => {
	withTestTempDirectory("builtin-policy-", (root) => {
		...
		const policy = unwrap(
			parseInitialBuiltinAccessPolicy({
				cwd: workspace,
				home,
				stateHome,
				piConfigDir,
				additionalSecretPatterns: ["secrets/custom.txt"],
			}),
		)
		...
		for (const secret of [privateKey, credentials, envFile, configuredSecret, piAuth]) {
			...
			if (!result.ok) assert.equal(result.error.kind, "secret-path")
		}
		...
		if (!protectedRead.ok) assert.equal(protectedRead.error.kind, "protected-snapshot-path")
		...
		for (const protectedWrite of [
			...
			if (!result.ok) assert.equal(result.error.kind, "protected-write-path")
		}
		...
		for (const immutable of [
			...
			if (!result.ok) assert.equal(result.error.kind, "immutable-write-root")
		}
	})
})
