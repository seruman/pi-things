...
import { parseProtectedPathPattern } from "./protected-path"
import { unwrap } from "./result"
import { canonicalPath } from "./test-domain-values"
import { withTestTempDirectory } from "./test-temp-directory"

function globPattern(pattern: ProtectedPathPattern): Extract<ProtectedPathPattern, { kind: "glob" }> {
	if (pattern.kind !== "glob") assert.fail(`expected glob pattern, received ${pattern.kind}`)
	return pattern
}

test("parses literal protected paths relative to a canonical root", () => {
	withTestTempDirectory("pi-safety-pattern-", (root) => {
		const protectedFile = path.join(root, "literal*.env")
		fs.writeFileSync(protectedFile, "secret")
		const parsed = unwrap(parseProtectedPathPattern("literal\\*.env", canonicalPath(root)))

		assert.equal(parsed.kind, "literal")
		if (parsed.kind === "literal") assert.equal(parsed.path, protectedFile)
	})
})
...
test("canonicalizes a glob's static base and compiles it with the library", () => {
	withTestTempDirectory("pi-safety-pattern-", (root) => {
		...
		const parsed = globPattern(unwrap(parseProtectedPathPattern("link/**/*.env", canonicalPath(root))))
...
test("supports component wildcards and character classes", () => {
	withTestTempDirectory("pi-safety-pattern-", (root) => {
		const parsed = globPattern(unwrap(parseProtectedPathPattern("*/file[0-9]?.txt", canonicalPath(root))))
		const regex = new RegExp(parsed.regex)
...
test("rejects patterns outside the intentionally supported subset", () => {
	withTestTempDirectory("pi-safety-pattern-", (root) => {
		...
		for (const [input, kind] of [
			...
			const parsed = parseProtectedPathPattern(input, base)
