				...
				await consumeSseJson({
					...
					onPayload: async (raw) => {
						...
						if (event.type === "response.output_item.done" && event.item) {
							...
							if (item.success) {
								const snapshot = item.data.content
									...
									.flatMap((c) => (c.type === "output_text" && typeof c.text === "string" ? [c.text] : []))
									.join("")
						...
						if (event.type === "response.failed") {
							...
							err.retryable =
								e?.code === "rate_limit_exceeded" || e?.code === "server_is_overloaded" || e?.code === "slow_down"
							throw err
						}
