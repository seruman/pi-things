...
async function parallelMap<TIn, TOut>(items: TIn[], concurrency: number, fn: (item: TIn, index: number) => Promise<TOut>) {
...
export default function (pi: ExtensionAPI) {
	...
	pi.registerCommand("subagents-running", {
		...
		handler: async (args, ctx) => {
			...
			const lines = items.slice(0, 40).map((item) => {
				const icon = item.status === "queued" ? "…" : item.status === "running" ? "▶" : item.status === "done" ? "✓" : item.status === "aborted" ? "■" : "✗";
	...
	pi.registerTool({
		...
		async execute(_toolCallId, params, signal, onUpdate, ctx) {
			const mode = Number(Boolean(params.task && !params.tasks?.length && !params.chain?.length)) + Number(Boolean(params.tasks?.length)) + Number(Boolean(params.chain?.length));
			...
			if (params.chain?.length) {
				...
				for (let i = 0; i < params.chain.length; i++) {
					...
					if (current.exitCode !== 0 || current.stopReason === "error" || current.stopReason === "aborted") {
						return {
							content: [{ type: "text", text: `Chain failed at step ${i + 1} (${item.agent}): ${current.final || current.stderr}` }],
			...
			if (params.tasks?.length) {
				...
				const summary = results
					.map((item) => `- [${item.agent}] ${item.exitCode === 0 ? "ok" : "failed"}: ${trim(item.final, 280) || "(no output)"}`)
					.join("\n");
				return {
		...
		renderCall(args, theme) {
			...
			return new Text(theme.fg("toolTitle", theme.bold("subagent ")) + theme.fg("accent", agent) + theme.fg("muted", ` [${scope}]`), 0, 0);
		},
		...
		renderResult(result, options, theme) {
			...
			if (!expanded) {
				...
				const rows = data.results.map((entry, index) => {
					const icon = entry.exitCode === -1 ? theme.fg("warning", marker(entry, tick)) : entry.exitCode === 0 ? theme.fg("success", marker(entry, tick)) : theme.fg("error", marker(entry, tick));
